...
export default function BeritaList() {
  ...
  async function handleCreateBerita(data: { title: string; description: string; image: File | null }) {
    try {
      ...
      if (data.image) {
        const storageRef = ref(storage, `berita/${Date.now()}_${data.image.name}`);
  ...
  return (
    <div className="p-6">
      <div className="mb-8 flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-800">Berita</h2>
        <button
      ...
      {loading ? (
        ...
        <div className="text-gray-500 text-center py-8">Belum ada Berita.</div>
      ) : (
